...
Production: Route53 -> ACM certificate -> ALB (via the controller) -> Service -> pods, with TLS terminated at the ALB. Remaining work to go live: install the controller, point DNS, and extend the NetworkPolicy ingress (ALB target-type `ip` sends traffic from ALB ENIs, which are not pods - the commented-out VPC-CIDR rule in `networkpolicy.yaml` covers this; a monitoring-namespace rule is likewise needed before Prometheus can scrape).
...
- **Fresh bootstrap starts with a nonexistent placeholder image.** The base Deployment ships tag `sha-0000000`, so immediately after `flux bootstrap` the app namespaces sit in ImagePullBackOff until the first build lands and the deploy/promotion PRs merge. This is a deliberate consequence of GitOps-only image delivery (no image exists before CI has ever run); the alternative, seeding overlays with a known-good tag, would put an unbuilt promise into Git. Expect `kubectl get events` to show pull failures for the first minutes of any re-bootstrap.
